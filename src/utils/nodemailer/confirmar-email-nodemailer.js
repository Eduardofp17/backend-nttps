...
const templatePath = path.join(__dirname, '..', 'templates', 'confirmar-email.ejs');
...
class NodeMailer {
  constructor(content) {
    ...
    this.emailLink = content.emailLink;
  ...
  async formatHtml(userName, emailLink) {
    return new Promise((resolve, reject) => {
      fs.readFile(templatePath, 'utf8', (err, data) => {
        ...
        this.html = String(ejs.render(data, { userName, emailLink }));
  ...
  async sendEmail() {
    try {
      await this.formatHtml(this.userName, this.emailLink);
